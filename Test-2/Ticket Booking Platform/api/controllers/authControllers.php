...
class AuthController
{
    ...
    public function register($firstName, $lastName, $email, $dob, $phoneno, $password, $role = 'user')
    {
        if ($this->isEmailExists($email)) {
            return sendResponse(false, "Email is already registered");
        }
        $hashedPwd = md5($password);
        $validRole = in_array($role, ['user', 'admin']) ? $role : 'user';
        $insertQuery = "INSERT INTO users 
        (first_name, last_name, email, dob, phone, password, role) 
        VALUES (:first_name, :last_name, :email, :dob, :phone, :password, :role)";
    ...
    {
        $query = "SELECT u.id, u.first_name, u.last_name, u.email, u.dob, u.phone, u.role, u.profile_image 
                  FROM users u
                  JOIN user_tokens ut ON ut.user_id = u.id
                  WHERE ut.token = :token AND ut.is_active = true";
        $this->db->query($query);
        $user = $this->db->first([':token' => $token]);
        if (!$user) {
            return sendResponse(false, "User not found or session expired");
        }
        return sendResponse(true, "Profile fetched", $user);
    }
    public function updateProfile($token, $firstName, $lastName, $dob, $phone)
    {
        $query = "SELECT user_id FROM user_tokens WHERE token = :token AND is_active = true";
        $this->db->query($query);
        $tokenRow = $this->db->first([':token' => $token]);
        if (!$tokenRow) {
            return sendResponse(false, "Invalid session");
        }
        $userId = $tokenRow['user_id'];
        $updateQuery = "UPDATE users 
                        SET first_name = :first_name, last_name = :last_name, dob = :dob, phone = :phone 
                        WHERE id = :id";
        ...
        return sendResponse(false, "Failed to update profile");
    }
    public function changePasswordUser($token, $currentPassword, $newPassword)
    {
        $query = "SELECT u.id, u.password FROM users u
                  JOIN user_tokens ut ON ut.user_id = u.id
                  WHERE ut.token = :token AND ut.is_active = true";
        $this->db->query($query);
        $user = $this->db->first([':token' => $token]);
        ...
        return sendResponse(false, "Failed to change password");
    }
    public function updateProfileImage($token, $imagePath)
    {
        $query = "SELECT user_id FROM user_tokens WHERE token = :token AND is_active = true";
        $this->db->query($query);
        $tokenRow = $this->db->first([':token' => $token]);
        ...
        $userId = $tokenRow['user_id'];
        $updateQuery = "UPDATE users SET profile_image = :image_path WHERE id = :id";
        $this->db->query($updateQuery);
        $result = $this->db->update([
        ...
        if ($result) {
            return sendResponse(true, "Profile image updated successfully", ['image_url' => $imagePath]);
        }
        return sendResponse(false, "Failed to update profile image");
    }
}
